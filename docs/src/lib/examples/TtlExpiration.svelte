...
<div class="flex w-full max-w-4xl flex-col gap-6 lg:flex-row">
    <!-- Controls Panel -->
    <div class="w-full rounded-xl border border-border bg-card p-6 shadow-sm lg:w-80">
        <h3 class="mb-4 text-lg font-semibold text-foreground">Add Entry with TTL</h3>

        <div class="space-y-4">
            <div>
                <label for="ttl-key" class="mb-1.5 block text-sm font-medium text-foreground">Key</label>
                <input
                    id="ttl-key"
                    type="text"
                    bind:value={inputKey}
                    placeholder="cache:key"
                    class="w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-brand-500 focus:outline-none"
                />
            </div>

            <div>
                <label for="ttl-value" class="mb-1.5 block text-sm font-medium text-foreground">Value</label>
                <input
                    id="ttl-value"
                    type="text"
            ...
            <div>
                <label for="ttl-slider" class="mb-1.5 block text-sm font-medium text-foreground">
            ...
            <div class="flex gap-2">
                <button
                    onclick={handleAdd}
                    class="flex-1 rounded-md bg-brand-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-brand-700"
                >
                    Add Entry
                </button>
                <button
        ...
        <div class="mt-6 rounded-lg bg-muted/50 p-4">
            <div class="flex justify-between text-sm">
                ...
                <span class="font-medium text-foreground">{entries.length}</span>
            </div>
    ...
    <div class="flex-1 rounded-xl border border-border bg-card p-6 shadow-sm">
        <div class="mb-4 flex items-center justify-between">
            <h3 class="text-lg font-semibold text-foreground">Cache Entries</h3>
            <span class="text-xs text-muted-foreground">
                Watch entries expire in real-time
            </span>
        </div>
        ...
        {:else}
            <div class="space-y-3">
                {#each entries as entry (entry.key)}
                    <div class="ttl-entry overflow-hidden rounded-lg border border-border bg-background">
                        <div class="flex items-center justify-between p-3">
                            ...
                            <div class="ml-3 text-right">
                                <div class="text-lg font-bold tabular-nums" class:text-red-500={entry.remainingMs < 3000} class:text-yellow-500={entry.remainingMs >= 3000 && entry.remainingMs < 6000} class:text-green-500={entry.remainingMs >= 6000}>
                        ...
                        <div class="h-1 w-full bg-muted">
                            <div
                                class="h-full transition-all duration-100 {getProgressColor(entry.remainingPercent)}"
