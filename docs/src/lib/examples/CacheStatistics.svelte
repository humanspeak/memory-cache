<script lang="ts">
    ...
    function getOperationColor(type: string): string {
        switch (type) {
            case 'HIT': return 'text-green-600 bg-green-500/10'
            case 'MISS': return 'text-red-600 bg-red-500/10'
            case 'SET': return 'text-blue-600 bg-blue-500/10'
            case 'DELETE': return 'text-orange-600 bg-orange-500/10'
            case 'EVICT': return 'text-purple-600 bg-purple-500/10'
...
<div class="flex w-full max-w-5xl flex-col gap-6">
    ...
    <div class="grid grid-cols-2 gap-4 md:grid-cols-5">
        ...
        <div class="rounded-xl border border-border bg-card p-4 text-center shadow-sm">
            ...
            <div class="text-sm text-muted-foreground">Sets</div>
        </div>
        <div class="rounded-xl border border-border bg-card p-4 text-center shadow-sm">
    ...
    <div class="flex flex-col gap-6 lg:flex-row">
        ...
        <div class="flex-1 rounded-xl border border-border bg-card p-6 shadow-sm">
            <h3 class="mb-4 text-lg font-semibold text-foreground">Hit Rate</h3>
            ...
            <div class="mb-6 flex justify-center">
                <div class="relative h-32 w-32">
                    ...
                    <div class="absolute inset-0 flex items-center justify-center">
                        <span class="text-2xl font-bold text-foreground">{hitRate.toFixed(1)}%</span>
                    </div>
                </div>
            </div>
        ...
        <div class="flex-1 rounded-xl border border-border bg-card p-6 shadow-sm">
            <div class="mb-4 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-foreground">Operation Log</h3>
                <span class="text-xs text-muted-foreground">Last 20 operations</span>
            </div>
            ...
            {:else}
                <div class="max-h-80 space-y-1.5 overflow-y-auto">
                    {#each operationLog as op, i (op.timestamp + i)}
                        <div class="log-entry flex items-center justify-between rounded-md border border-border bg-background px-3 py-2">
                            <div class="flex items-center gap-2">
                                <span class="rounded px-2 py-0.5 text-xs font-bold {getOperationColor(op.type)}">
                                    {op.type}
                                </span>
                                <code class="font-mono text-sm text-foreground">{op.key}</code>
                            </div>
